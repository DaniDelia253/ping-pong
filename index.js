...
const RoleIDs = {
	...
	ServerGiveAway: "1158104332340826224",
	ServerButterfly: "1158103045591289876",
	ServerBooster: "1158103388106534962",
	ServerHigherLevel: "1158103545174831135",
};
...
client.on("guildMemberUpdate", async (oldMember, newMember) => {
	...
	const listOfRolesToCheck = [RoleIDs.ServerButterfly, RoleIDs.ServerBooster];
	listOfRolesToCheck.forEach((role) => {
		if (
			...
			newMember.roles.add(RoleIDs.ServerGiveAway);
		}
	});

	let listOfHigherRoles = [
		RoleIDs.ServerButterfly,
		RoleIDs.ServerHigherLevel,
	];
	...
	if (
		...
		oldMember.roles.cache.has(RoleIDs.ServerBooster) &&
		// AND newMember does not have booster
		!newMember.roles.cache.has(RoleIDs.ServerBooster) &&
		...
		newMember.roles.remove(RoleIDs.ServerGiveAway);
	}
});
...
client.on("messageCreate", async (message) => {
	...
	if (message.channelId === ChannelIDs.ServerDizzyPlant) {
		//before comparing the phrases, convert the text that the person typed to all lowercase
